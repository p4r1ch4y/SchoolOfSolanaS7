...
describe("daily_idea_spark_funded", () => {
...
Test account that needs funding: ${keypair.publicKey.toString()}
  ...
  describe("initialize_journal", () => {
    it("1. Happy: Successfully initializes a new journal", async () => {
    ...
    it("2. Unhappy: Fails when trying to initialize journal that already exists", async () => {
      const [journalPDA] = getJournalPDA(user1.publicKey);
  ...
  describe("log_idea", () => {
    it("3. Happy: Successfully logs first idea and sets streak to 1", async () => {
    ...
    it("4. Happy: Increments streak when logging within 24 hours", async () => {
      const [journalPDA] = getJournalPDA(user2.publicKey);
      ...
      expect(journalAccount.ideas[1].timestamp.toNumber()).to.be.greaterThan(journalAccount.ideas[0].timestamp.toNumber());

      console.log("Second idea logged, streak incremented:", tx);
    });

    it("5. Unhappy: Fails when idea text is empty", async () => {
      const [journalPDA] = getJournalPDA(user2.publicKey);
    ...
    it("6. Unhappy: Fails when idea text is too long", async () => {
      const [journalPDA] = getJournalPDA(user2.publicKey);
  ...
  describe("get_streak", () => {
    it("7. Happy: Successfully retrieves current streak", async () => {
      const [journalPDA] = getJournalPDA(user2.publicKey);
    ...
    it("8. Happy: Demonstrates account isolation between users", async () => {
    ...
    it("9. Happy: Ideas array properly maintains limit of 20 entries", async () => {
